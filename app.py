...
def descargar_word():
    ...
    try:
        ...
        def generar_docx_primaria(doc, datos, sesion):
            ...
            # Generate relevant educational resources based on topic and level
            tema = datos.get('tema', 'Educación')
            nivel = datos.get('nivel', 'Primaria')
            area = datos.get('area', 'General')
            
            recursos_educativos = f"""📚 RECURSOS EDUCATIVOS DIGITALES

🎥 Videos Educativos sobre "{tema}":
• YouTube Educativo: https://www.youtube.com/results?search_query={tema.replace(' ', '+')}+{nivel}+Peru
• PerúEduca Videos: https://www.perueduca.pe/
• Khan Academy en Español: https://es.khanacademy.org/

🖼️ Imágenes y Material Visual:
• Google Imágenes Educativas: https://www.google.com/search?q={tema.replace(' ', '+')}+educacion+{nivel}&tbm=isch
• Canva Educación: https://www.canva.com/es_es/educacion/
• Banco de Imágenes MINEDU: http://www.minedu.gob.pe/

📖 Materiales Complementarios:
• Plataforma PerúEduca: https://www.perueduca.pe/
• Currículo Nacional (CNEB): http://www.minedu.gob.pe/curriculo/
• Recursos para {area}: https://www.google.com/search?q=recursos+{area.replace(' ', '+')}+{nivel}+Peru

💡 Herramientas Digitales:
• Jamboard (Pizarra colaborativa): https://jamboard.google.com/
• Padlet (Murales interactivos): https://padlet.com/
• Kahoot (Evaluaciones interactivas): https://kahoot.com/

✨ Generado con IA para optimizar tu planificación pedagógica."""
            
            add_normal_text(c_rec, recursos_educativos, 9)
